$(function(){
    ...
    var base_url='http://192.168.0.252/lessential/'; // Here i define the base_url
    ...
    $('#calendar').fullCalendar({
        ...
        dayClick: function(date, event, view) {
         ...
         eventDrop: function(event, delta, revertFunc) {  
               ...
               $.post(base_url+'dashboard/dragUpdateEvent',{                            
                id:event.id,
    ...
    $('.modal').on('click', '#add-event',  function(e){
        if(validator(['title', 'description'])) {
            $.post(base_url+'dashboard/addEvent', {
                title: $('#title').val(),
    ...
    $('.modal').on('click', '#update-event',  function(e){
        if(validator(['title', 'description'])) {
            $.post(base_url+'dashboard/updateEvent', {
                id: currentEvent._id,
                title: $('#title').val(),
                description: $('#description').val(),
                color: $('#color').val(),
    ...
    $('.modal').on('click', '#delete-event',  function(e){
        $.get(base_url+'dashboard/deleteEvent?id=' + currentEvent._id, function(result){
            $('.modal').modal('hide');
            $('#calendar').fullCalendar("refetchEvents");
        });
    });
